...
# SIBOT1 is ROS MASTER
#   Interface: is eth0
#   IP address is 10.66.171.4 
...
if [ "$HOSTNAME" = SIBOT1 ]; then
    ...
    then
        ...
        export ROS_IP=$(ip -4 address show $ROBOT_NETWORK | grep 'inet' | sed 's/.*inet \([0-9\.]\+\).*/\1/')
        #now lets try to ping sibot1 to see if it should be the master
        ping -q -c 1 -W 1 sibot1 >/dev/null
        temp=$?
        if [ $temp -ne 0 ]; then
            #Could not find sibot1 even though we are on the right network just make this PC master
            export ROS_MASTER_URI=http://$ROS_IP:11311/
        else
            #Found sibot1 so lets use sibot1 and the master_uri 
            export ROS_MASTER_URI=http://SIBOT1:11311/
        fi
    else
        #No interface on the sibot1 network; default simulation settings
